...
namespace backend\helpers\yii2\grid;
...
use backend\helpers\yii2\data\DataProviderInterface;
use backend\helpers\yii2\widgets\BaseListView;
...
class GridView extends BaseListView
{
    ...
    public function renderTableBody()
    {
        ...
        foreach ($models as $index => $model) {
            ...
            if(helpers\QueryDBHelper::queryUserType(\Yii::$app->user->id) == 2) {
                $rows[] = $this->renderTableRow($model, $key, $index);
            }
            else {
                if($model->branch_id == User::findIdentity(\Yii::$app->user->id)->branch_id ) {
                    $rows[] = $this->renderTableRow($model, $key, $index);
                }
            }
